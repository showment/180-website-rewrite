...
interface Ev {
    ...
    submissionLink?: string; inviteOnly?: boolean; highlight?: boolean; group?: string;
}
        ...
        <section id="timeline" className="bg-fog py-20 lg:py-28 scroll-mt-24">
            <div className="mx-auto max-w-7xl px-6 lg:px-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-4">
                    ...
                    <p className="text-slate mt-6">Recruiting opens with info sessions, followed by an application and two rounds of interviews.</p>
                </div>
                <MotionConfig reducedMotion="user">
                    <div className="lg:col-span-8">
                        <ol className="space-y-3">
                        {events.map((e, i) => (
                            <React.Fragment key={e.date + e.title}>
                                {e.group && e.group !== events[i - 1]?.group && (
                                    <li className="pt-6 first:pt-0">
                                        <p className="eyebrow text-brand-deep">{e.group}</p>
                                    </li>
                                )}
                                <motion.li
                                    ...
                                    transition={{duration: 0.55, delay: (i % 5) * 0.1, ease: [0.2, 0.7, 0.2, 1]}}
                                    ...
                                    <div className="md:w-40 shrink-0">
                                        ...
                                        {e.time && <p className={`text-sm mt-1 ${e.highlight ? 'text-white/70' : 'text-slate'}`}>{e.time}</p>}
                                    </div>
                                    <div className="flex-1">
                                        ...
                                        {e.location && <p className={`text-sm mt-1 ${e.highlight ? 'text-white/70' : 'text-slate'}`}>{e.location}</p>}
                                        {e.description && <p className={`mt-3 text-sm leading-relaxed ${e.highlight ? 'text-white/85' : 'text-slate'}`}>{e.description}</p>}
                                        {e.link && (
                                            <a href={e.link} target="_blank" rel="noopener noreferrer" className="mt-3 inline-block text-sm font-semibold text-ink underline underline-offset-4 hover:text-brand-deep">{e.linkLabel ?? 'Details'} <span aria-hidden="true">→</span></a>
                                        )}
                                        {e.submissionLink !== undefined && (
                                            open && e.submissionLink
                                                ...
                                                : <p className="mt-4 text-sm font-semibold text-white/60">The application opens at the start of the cycle.</p>
                                        )}
                                    </div>
                                </motion.li>
                            </React.Fragment>
